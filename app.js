...
window.onload = () => {
  ...
  // Get unique modules from the QUESTIONS array
  const uniqueModules = [...new Set(QUESTIONS.map(q => q.module))];
  uniqueModules.sort();

  // Add "All Topics" option to the front
  ...
  uniqueModules.forEach(module => {
    ...
    // UI Fix: Make "ClinicalJudgment" look like "Clinical Judgment"
    let displayLabel = module === "All" ? "All Topics (Simulated Exam)" : module;
    if (module === "ClinicalJudgment") displayLabel = "Clinical Judgment";
    ...
    btn.onclick = () => {
      ...
      // Scroll to the bottom so the "Start" button is visible on mobile
